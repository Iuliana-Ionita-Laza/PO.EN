...
#'                    AUROC (\code{type.measure='auc'}) and F measure (\code{type.measure='F'}).
...
cv.PO.EN<- function(X, Y, alpha=0.5, o.iter=5, i.iter=20, epsilon=1e-4,nfolds=5,type.measure='deviance',depth=100,input.pi=0.5,a=sqrt(0.5),seed=1){
  set.seed(seed = seed)
  ...
  repeat{
    ...
    Y<-Y[index.shuffle]
    indd<-aggregate(Y,by=list(rep(1:nfolds,each=floor(length(Y)/nfolds))),FUN='sum')
    if(all(indd[,2]!=0)){
      break
    }
  }
  ...
  start.function<-function(x,y,type.measure=type.measure,start.pi){
    ...
    # y<-Y
      ...
      glm.y<-function(h){
        en.fit<-glmnet(cbind(1,h),y,family='binomial',intercept=F,lambda=0)
      ...
      glm.re<-function(h){
        glm.re<-glm.fit(h,y,family=binomial(),intercept = T)
    ...
    repeat{
      start.s<-PO.EN(x,y,alpha = alpha,o.iter=10, i.iter=5, lambda=lambda.start,true.prob=start.pi,beta_start=rep(0,p+1),epsilon=epsilon,gram.input = T,XtX.input = XtX.start.i,ytx.input=ytx.start.i,XtX_reduce.input =  XtX_reduce.start.i)
    ...
    repeat{
      start.e<-PO.EN(x,y,alpha = alpha,o.iter=10, i.iter=5, lambda=lambda.run[length(lambda.run)],true.prob=start.pi,beta_start=rep(0,p+1),epsilon=epsilon,gram.input = T,XtX.input = XtX.start.i,ytx.input=ytx.start.i,XtX_reduce.input =  XtX_reduce.start.i)
      # print(lambda.run[length(lambda.run)])
      #   print(t(start.e))
    ...
    repeat{
      ...
      test.fit<-PO.EN(x,y,alpha = alpha,o.iter=10, i.iter=10, lambda=lambda.run[s],true.prob=start.pi,beta_start=rep(0,p+1),epsilon=epsilon,gram.input = T,XtX.input = XtX.start.i,ytx.input=ytx.start.i,XtX_reduce.input =  XtX_reduce.start.i)
      if(sum(test.fit[2:ncol(x)]!=0)!=0){
        break
      }
      s=s+1
    #  print(paste0('this is s:',s))
    }
  ...
  for(j in 1:pi.length){
    ...
    for(i in 1:nfolds){
      ...
      fit<-PO.EN(X.train,Y.train,alpha = alpha,o.iter=o.iter, i.iter=i.iter, lambda=lambda_run_list[[j]],true.prob=input.pi[j],beta_start=rep(0,p+1),epsilon=epsilon,gram.input = T,XtX.input = XtX.i,ytx.input=ytx.i,XtX_reduce.input = XtX_reduce.i)


      X.t<-scale(as.matrix(cbind(X))[valid.index,])
      Y.t<-Y[valid.index]

      #  result.matrix[,i]<-deviances(X.t,Y.t,input.pi[j],fit)
      result.matrix.dev[,i]<-deviances(X.t,Y.t,input.pi[j],fit)

      predictions<-PO.EN.predict(X.t,fit)
      result.matrix.auc[,i]<-apply(predictions,2,function(x){auc(Y.t,x,quiet=T)})
      predictions<-PO.EN.predict(X.t,fit)
  ...
  for(j in pi.index){
    ...
    if(type.measure=='deviance'){
      ...
      sd.run<-apply(result.matrix,1,sd)/sqrt(nfolds)
  ...
  # if(type.measure=='deviance'){
  #   result.pi<-input.pi[which.min(result.measure)]
  #   lambda.min<-result.min[which.min(result.measure)]
  #   lambda.1se<-result.1se[which.min(result.measure)]
  #   }
  #
  # if(type.measure=='auc'){
  #   result.pi<-input.pi[which.max(result.measure)]
  #   lambda.min<-result.min[which.max(result.measure)]
  #   lambda.1se<-result.1se[which.max(result.measure)]
  # }
  # if(type.measure=='F'){
  #   result.pi<-input.pi[which.max(result.measure)]
  #   lambda.min<-result.min[which.max(result.measure)]
